...
#' @param measure.name A optional character string naming the measure being evaluated, which is printed in the report.
...
#' @param match.type Οne of \code{c("Total", "Rest")}. Determines whether the total score or rest score should be used as the stratifying variable for loess, MH, and logistic regression methods. Defatul is "Total".
#' @param match.bins An optional vector of bin sizes for stratifying the matching variable in
...
summary_report <- function(dif.data,
  ...
  if(!(biased.items %in% dif.methods)) {
