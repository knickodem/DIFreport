...
**Outcome variable:** `r paste0(measure.name,  " (", item.name.range, ")")`
...
if(nvi != "none"){
  ...
  cat("^a^ Items were not included in any DIF analysis and were not used to compute 
      IRT-based robustness checks of treatment effects.")
  cat("\n\n")
}
...
if(nvgi != "none"){
  ...
  cat("^b^ Items were not included in the IRT-based DIF analysis and were not used 
      to compute IRT-based robustness checks of treatment effects.")
  cat("\n\n")
}
...
if(n.biased > 0){
  ...
  if(is.null(tx.group)){
    ...
    cat("Interaction of treatment effect by ", dif.group.name, 
        " (", dif.group1, " - ", dif.group2, ")", sep = "")
    cat("\n\n")
    inttab <- format_flex(interaction.effects)
...
if(!is.null(dif.analysis$MH)){
  ...
  if(class(dif.analysis$MH$item.level) == "character"){
    ...
    MHtab <- format_flex(dif.analysis$MH$Item, boldbias = "item")
...
if(!is.null(dif.analysis$logistic)){
  ...
  logglobtab <- format_flex(dif.analysis$logistic$global.level, boldbias = "global")
  ...
  if(class(dif.analysis$logistic$item.level) == "character"){
    ...
    logitemtab <- format_flex(dif.analysis$logistic$item.level, boldbias = "item")
...
if(!is.null(dif.analysis$IRT)){
  ...
  irtglobtab <- format_flex(dif.analysis$IRT$global.level, boldbias = "global")
  ...
  if(class(dif.analysis$IRT$item.level) == "character"){
    ...
    irtitemtab <- format_flex(dif.analysis$IRT$item.level, boldbias = "item")
