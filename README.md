...
**WBdif** documentation refers to these as unconditional and conditional
...
If you are interested in assessing DIF with respect to the treatment
variable and evaluating robustness of the unconditional treatment
effects, only `dif.group.id` need be supplied to `dif_data_prep()`.
...
variable (e.g., gender) and evaluating robustness of the conditional
treatment effects (e.g., for just females, for just males, and their
difference), then both `dif.group.id` and `tx.group.id`need to be
...
conditional treatment effect example in more detail.

The first recommended step is still `dif_data_prep()`, which operates in
the same manner as the [Basic workflow](#basic). This step is repeated
below.
...
used with multi-category (“polytomous”) assessment items.
  ...
  - Mantel-Haenszel (MH) Test - *only available for dichotomous items* -
    ...
    remedy is to bin the stratifying variable (e.g., by deciles), as
  ...
  - Item Response Theory (IRT) - First conducts an omnibus test of DIF
    ...
    re-test any of items dropped in the Purification stage. The same
    functionality can be achieved with `mirt::DIF()` using the
...
comparison of the DIF methods with one method highlighted via the
`biased.items` argument ([example
report](https://htmlpreview.github.io/?https://github.com/knickodem/WBdif/blob/master/README%20Example%20Reports/DIF-Only-Gender-MDAT-Language.html)).
...
can be the name one of the DIF methods run by `dif_analysis()`, in which
...
clustering based on the `clusterid` variable from the `mdat` data. Some
supporting plots for Some support functions for `effects_robutness`
...
| :----------- | --------------: | -----: | ----------------------: | -------------: |
| All Items    |            0.14 |   0.08 |                    0.15 |           0.08 |
| Bias Omitted |            0.14 |   0.08 |                    0.14 |           0.08 |
...
| Items        | IRT effect size | IRT SE | Total score effect size | Total score SE |
| :----------- | --------------: | -----: | ----------------------: | -------------: |
| All Items    |          \-0.02 |   0.07 |                       0 |           0.07 |
| Bias Omitted |          \-0.02 |   0.07 |                       0 |           0.07 |
...
| Items        | IRT effect size | IRT SE | Total score effect size | Total score SE |
| :----------- | --------------: | -----: | ----------------------: | -------------: |
| All Items    |          \-0.16 |   0.11 |                  \-0.15 |           0.11 |
| Bias Omitted |          \-0.16 |   0.11 |                  \-0.14 |           0.11 |
...
`effect_report()`. More often, the DIF analysis results are also of
interest, in which case use `dif_effect_report()` to include summaries
of both the DIF analysis and the treatment effect robustness checks,
such as the [report generated
here](https://htmlpreview.github.io/?https://github.com/knickodem/WBdif/blob/master/README%20Example%20Reports/Logistic-Gender-MDAT-Language.html)
